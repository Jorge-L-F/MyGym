        ...
        <v-card class="rounded pa-4 elevation-3">
          ...
          <v-form ref="updatePlaylistForm">
            <v-text-field
              ...
              name="playlistURL"
              type="text"
              :rules="[rules.required, rules.validSpotifyLink, rules.alreadyOnList]"
              v-model="spotifyLink"
              outlined
              prepend-inner-icon="person"
            ></v-text-field>

            <v-btn block class="mt-2 bg-blue" @click="updatePlaylists" outlined
              >Submit</v-btn
            >
          </v-form>
        </v-card>
    ...
    return {
      ...
      rules: {
        ...
        validSpotifyLink: (value) =>
          value.includes("https://open.spotify.com/") || "Invalid link.",
        alreadyOnList: (value) =>
          this.me.playlists.includes(value) || "Already on this list.",
      },
      spotifyLink: "",
    };
  },
 ...
 */
      ...
      fetch("https://open.spotify.com/oembed?url=" + playlist, requestOptions)
        .then((response) => response.text())
        .then((result) => {
          this.embedPlaylists = [...this.embedPlaylists, JSON.parse(result)];
